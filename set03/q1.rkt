...
;;; CONSTRUCTOR TEMPLATE
;;; A Racket is a (make-racket Integer Integer Integer Integer)
...
(define R-TEST (make-racket 50 60 70 80))

(define B-COLLIDE-R (make-ball 350 360 0 10))
(define R-COLLIDE-B (make-racket 350 365 5 0))

(define B-MISS-R (make-ball 350 360 0 10))
(define R-MISS-B (make-racket 350 365 0 10))

(define B-COLLIDE-RIGHT (make-ball 420 360 10 0))
(define R-COLLIDE-RIGHT (make-racket 400 360 10 0))

(define B-COLLIDE-LEFT (make-ball 5 360 -10 0))
(define R-COLLIDE-LEFT (make-racket 30 360 -10 0))

(define B-COLLIDE-TOP (make-ball 350 5 0 -10))
(define R-COLLIDE-TOP (make-racket 350 5 0 -10))

(define B-COLLIDE-BOTTOM (make-ball 350 645 0 10))
(define R-COLLIDE-BOTTOM (make-racket 350 645 0 10))

(define B-RTS (make-ball RTS-X RTS-Y RTS-VX RTS-VY))
(define R-RTS (make-racket RTS-X RTS-Y RTS-VX RTS-VY))
...
;;; (racket-vx (make-racket 10 20 30 40)) -> 30
;;; (racket-vy (make-racket 10 20 30 40)) -> 40
;;; DESIGN STRATEGY:
;;; Use observer template
...
(define (initial-world speed)
  (make-world (make-ball RTS-X RTS-Y RTS-VX RTS-VY)
              (make-racket RTS-X RTS-Y RTS-VX RTS-VY)
              speed
              true
              false
              0))
...
;;; EXAMPLE:
;;; (paused-racket W-TEST) => (make-racket 50 60 0 0)
...
;;; TESTS:
(begin-for-test
  (check-equal? (paused-racket W-TEST) (make-racket 50 60 0 0)
...
;;; EXAMPLE:
;;; (new-racket W-NO-COLL) => (make-racket 350 375 0 10)
;;; DESIGN STRATEGY: Constructor template for Racket
...
;;; TESTS
(begin-for-test
  (check-equal? (new-racket W-NO-COLL) (make-racket 350 375 0 10)
...
;;; (new-racket-vy B-COLLIDE-R (make-racket 350 365 5 -1)) => 0
...
(begin-for-test
  ...
  (check-equal? (new-racket-vy B-COLLIDE-R (make-racket 350 365 5 -1)) 0
