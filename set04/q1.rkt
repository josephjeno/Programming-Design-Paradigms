...
;;; DESIGN STRATEGY: Cases on empty list and then transcribe formula
...
;;; DESIGN STRATEGY: Cases on length of lists
...
(begin-for-test
  ...
  (check-equal? (permutation-of-equal-list? (list 3 1 2) (list 1 2 4)) false
                "permutation-of-equal-list? test 3 expected false as result")
  (check-equal? (permutation-of-equal-list? (list) (list)) true
                "permutation-of-equal-list? test 4 expected true as result"))
...
(begin-for-test
  (check-equal? (shortlex-less-than-equal-length? (list) (list)) false
                "shortlex-less-tahn-equal-length? test 1 expected false")
...
;;;                   (list 3 1 2)
;;;                   (list 3 2 1))
;;; DESIGN STRATEGY: Cases on length of given IntList
...
;                                  (list))]))
;
;(define (permutations-lists l i add ill)
;  (cond [(= (length l) 2) (permutations-lists (permutations-iterate
;                                 (cons add (first ill)))
;                                i
;                                empty
;                                (appendlist add (list (list (first l)
;                                (second l))
;                                (list (second l) (first l))))]
;        [(= (length ill) i) ill]
;        [else (append (permutations-lists (rest l)
;                                  i
;                                  (append add (list (first l)))
;                                  ill)
;                      (permutations-lists (permutations-iterate ]))
;
;;;; permutations-iterate-primary
;(define (permutations-iterate l i t ill)
;  (cons (list-ref l (- (length l) (- i 1)))
;        (mysort (remove (list-ref l (- (length l) (- i 1))) l))))
;
;
;(define (appendlist n l)
;  (cond [(empty? l) empty]
;        [else (cons (append n (first l))
;                    (appendlist n (rest l)))]))