...
;;; world-to-scene-balls : World -> Scene
;;; GIVEN: a World that does not have a selected racket
;;; RETURNS: a Scene that portrays the given world
;;; DESIGN STRATEGY: Place items in world.
...
(define (world-reset w)
  (if (< (world-plength w) RALLY-PAUSE-lENGTH )
      (make-world (paused-balls (world-balls w))
                  (paused-racket w)
                  (world-speed w)
                  (world-ready-to-serve? w)
                  true
...
;;; paused-balls : World -> Ball
;;; GIVEN: A paused world
;;; RETURNS: The balls in that world with no velocity
;;; EXAMPLE:
;;; (paused-ball W-TEST) => (make-ball 10 20 0 0)
;;; DESIGN STRATEGY: Constructor template for Ball

(define (paused-balls balls)
  (cond
    [(empty? balls) empty]
    [else (cons (make-ball (ball-x (first balls))
                           (ball-y (first balls))
                           0
                           0)
                (paused-balls (rest balls)))]))
  
;;; TESTS:
(begin-for-test
  (check-equal? (paused-balls (list B-TEST)) (list (make-ball 10 20 0 0))
...
(define (next-world w)
  (make-world (new-balls (world-balls w) (world-racket w))
              (new-racket w)
              (world-speed w)
              (world-ready-to-serve? w)
              (world-paused? w)
...
(define (new-racket w)
  (make-racket (new-racket-x (world-balls w) (world-racket w))
               (new-racket-y (world-balls w) (world-racket w))
               (new-racket-vx (world-balls w) (world-racket w))
               (new-racket-vy (world-balls w) (world-racket w))
...
(define (new-racket-y b r)
  (cond [(empty? b) (if (racket-selected? r)
                        (racket-y r)
                        (+ (racket-y r) (racket-vy r)))]
        [(ball-collide-racket? (first b) r) (+ (racket-y r) (racket-vy r))]
        [else (new-racket-y (rest b) r)]))
