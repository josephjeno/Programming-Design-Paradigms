...
;;; -- empty              represents 0
;;; -- (cons b bl)
;;;         WHERE b is a Ball and bl is a BallList
...
(define B-RALLY (make-ball RTS-X RTS-Y RALLY-VX RALLY-VY))
...
;;;    (make-world balls racket speed ready-to-serve? paused? plength)
;;; with the following fields:
;;;           balls : BallList any valid BallList
...
;;; IMPLEMENTATION
(define-struct world (balls racket speed ready-to-serve? paused? plength))

;;; CONSTRUCTOR TEMPLATE
;;; A World is a (make-world BallList Racket PosReal Boolean Boolean PosReal)
...
;;; used for testing
(define W-TEST (make-world (list B-TEST) R-TEST 1 true true 0))
(define W-PAUSED (make-world (list B-TEST) R-TEST 1 false true 0))
(define W-RTS (make-world (list B-RTS) R-RTS 1 true false 0))
(define W-NO-COLL (make-world (list B-MISS-R) R-MISS-B 1 false false 0))
(define W-B-COLL-BOTTOM (make-world (list B-COLLIDE-BOTTOM)
                                    R-TEST 1 false false 0))
(define W-R-COLL-BOTTOM (make-world (list B-TEST)
                                    R-COLLIDE-BOTTOM 1 false false 0))
(define W-R-COLL-TOP (make-world (list B-TEST) R-COLLIDE-TOP 1 false false 0))
...
;;; TESTS:
(begin-for-test
  (check-equal? (world-balls W-TEST) (list B-TEST)
                "world-balls test 1 did not calculate correctly"))
...
(define (world-to-scene w)
  (if (racket-selected? (world-racket w))
      (world-to-scene-selected w)
      (world-to-scene-balls w (world-balls w))))
...
(begin-for-test
  ...
  (check-equal? (world-to-scene
                 (make-world (list B-TEST)
                             (make-racket 50 60 70 80 true 0 0) 1 true true 0))
                (place-image MOUSE 50 60
                (place-image BALL 10 20
                             (place-image RACKET 50 60 PAUSED-COURT)))
                "world-to-scene test 3 did not calculate correctly"))


;;; world-to-scene-selected : World -> Scene
;;; GIVEN: a World with a racket that is currently selected
;;; RETURNS: a Scene that portrays the given world (including mouse pointer)
;;; EXAMPLE:
;;; (world-to-scene-selected W-TEST should return a white canvas with a ball 
;;; placed at (10,20) and a racket placed at (50,60)
;;; (world-to-scene-selected W-RTS should return a yellow canvas with a ball 
;;; placed at (330,384) and a racket placed at (330,384)
;;; DESIGN STRATEGY: Place items in world.

(define (world-to-scene-selected w)
  (place-image MOUSE
...
;;; TESTS:
(begin-for-test
  (check-equal? (world-to-scene-selected
                 (make-world (list B-TEST) R-TEST 1 true true 0))
                (place-image MOUSE 50 60
                (place-image BALL 10 20
                             (place-image RACKET 50 60 PAUSED-COURT)))
                "world-to-scene-selected test 1 did not calculate correctly"))


;;; world-to-scene-balls : World BallList -> Scene
;;; GIVEN: a World that does not have a selected racket and a BallList
;;; RETURNS: a Scene that portrays the given world
;;; EXAMPLE:
;;; (world-to-scene-selected W-TEST should return a white canvas with a ball 
;;; placed at (10,20) and a racket placed at (50,60)
...
(define (world-to-scene-balls w balls)
  (cond
    [(empty? balls) (world-to-scene-racket-court w)]
     [else (place-image BALL
                        (ball-x (first balls))
                        (ball-y (first balls))
                        (world-to-scene-balls w (rest balls)))]))

;;; TESTS:
(begin-for-test
  (check-equal? (world-to-scene-balls W-TEST (list B-TEST))
                (place-image BALL 10 20
                             (place-image RACKET 50 60 PAUSED-COURT))
                "world-to-scene-balls test 1 did not calculate correctly")
  (check-equal? (world-to-scene-balls W-RTS (list B-RTS))
                (place-image BALL 330 384
                             (place-image RACKET 330 384 EMPTY-COURT))
                "world-to-scene-balls test 2 did not calculate correctly"))


;;; world-to-scene-racket-court : World -> Scene
;;; GIVEN: a World without balls
;;; RETURNS: a Scene that portrays the given world
;;; EXAMPLE:
;;; (world-to-scene-racket-court W-TEST) returns a white canvas with a
;;; racket placed at (50,60)
;;; DESIGN STRATEGY: Place items in world.

(define (world-to-scene-racket-court w)
  (place-image RACKET (racket-x (world-racket w))
               (racket-y (world-racket w))
               (if (world-paused? w)
                   PAUSED-COURT EMPTY-COURT)))

;;; TESTS:
(begin-for-test
  (check-equal? (world-to-scene-racket-court W-TEST)
                (place-image RACKET 50 60 PAUSED-COURT))
                "world-to-scene-racket-court test 1 not correct")
...
;;; EXAMPLE:
;;; (initial-world 1) -> (make-world (list B-RTS) R-RTS 1 true false 0)
;;; DESIGN STRATEGY: Follow constructor template for World

(define (initial-world speed)
  (make-world (list (make-ball RTS-X RTS-Y RTS-VX RTS-VY))
...
;;; TESTS:
(begin-for-test
  (check-equal? (initial-world 1) (make-world (list B-RTS) R-RTS 1 true false 0)
...
(define (world-after-tick w)
  (cond [(world-ready-to-serve? w) w]
        ...
        [(= (length (world-balls w)) 0) (world-reset w)]
        [else (next-world w)]))
...
(begin-for-test
  ...
  (check-equal? (world-after-tick (make-world empty R-TEST 1 false false 0))
                                  (world-reset
                                   (make-world empty R-TEST 1 false false 0))
...
;;; DESIGN STRATEGY: Cases on plength, if paused use HOF map to make new world
;;;                  with paused balls

(define (world-reset w)
  (if (< (world-plength w) RALLY-PAUSE-lENGTH )
      (make-world (map paused-balls (world-balls w))
                  (paused-racket (world-racket w))
                  (world-speed w)
                  (world-ready-to-serve? w)
                  true
...
;;; paused-balls : Ball -> Ball
;;; GIVEN: A ball 
;;; RETURNS: The balls with no velocity
;;; EXAMPLE:
;;; (paused-ball B-TEST) => (make-ball 10 20 0 0)
;;; DESIGN STRATEGY: Constructor template for Ball

(define (paused-balls b)
  (make-ball (ball-x b)
             (ball-y b)
             0
             0))
...
;;; TESTS:
(begin-for-test
  (check-equal? (paused-balls B-TEST) (make-ball 10 20 0 0)
                "paused-balls test 1 did not calculate correctly"))

;;; paused-racket : Racket -> Racket
;;; GIVEN: A racket
;;; RETURNS: The racket with no velocity
;;; EXAMPLE:
;;; (paused-racket R-TEST) => (make-racket 50 60 0 0 false 0 0)
;;; DESIGN STRATEGY: Constructor template for Racket

(define (paused-racket r)
...
;;; TESTS:
(begin-for-test
  (check-equal? (paused-racket R-TEST) (make-racket 50 60 0 0 false 0 0)
...
;;;                                             false
;;;                                             0)
;;; DESIGN STRATEGY: Constructor template for World using HOF map on a list
;;;                  made by HOF filter

(define (next-world w)
  (make-world (map
               ;; Ball -> Ball
               ;; RETURNS: the next location of the ball
               (lambda (x) (new-ball x (world-racket w)))
               (filter ball-collide-back? (world-balls w)))
              (new-racket (world-balls w) (world-racket w))
              (world-speed w)
              (world-ready-to-serve? w)
              (world-paused? w)
              (world-plength w)))
...
(begin-for-test
  (check-equal? (next-world W-NO-COLL)
                (make-world (list (make-ball 350 370 0 10))
...
;;; new-balls : BallList Racket -> BallList
;;; GIVEN: a world in a rally state
;;; RETURNS: the BallList in that world after 1 tick
;;; EXAMPLE:
;;; (new-ball W-NO-COLL) => (make-ball 350 370 0 10)
;;; DESIGN STRATEGY: Constructor template for Ball
;;;
;;;
;;; (define (new-balls b r)
;;;   (cond
;;;     [(empty? b) empty]
;;;     [(ball-collide-back? (first b)) (new-balls (rest b) r)]
;;;     [else (cons (new-ball (first b) r)
;;;                 (new-balls (rest b) r))]))
;;;
;;; TESTS
;;;
;;; (begin-for-test
;;;   (check-equal? (new-balls (list B-MISS-R) R-MISS-B)
;;;                 (list (make-ball 350 370 0 10))
;;;                 "new-ball test 1 did not calculate correctly")
;;;   (check-equal? (new-balls (list (make-ball 200 645 0 10)) R-MISS-B)
;;;                 (list)
;;;                 "new-ball test 1 did not calculate correctly"))


;;; new-ball : Ball Racket -> Ball
;;; GIVEN: a world in a rally state
...
;;; TESTS
(begin-for-test
  (check-equal? (new-ball B-MISS-R R-MISS-B) (make-ball 350 370 0 10)
                "new-ball test 1 did not calculate correctly"))
...
;;; new-racket : BallList Racket -> Racket
;;; GIVEN: a BallList and a Racket
;;; RETURNS: the Racket after 1 tick
;;; EXAMPLE:
;;; (new-racket (list B-NO-COLL) R-NO-COLL) => (make-racket 350 375 0 10 false)
;;; DESIGN STRATEGY: Constructor template for Racket

(define (new-racket b r)
  (make-racket (new-racket-x b r)
               (new-racket-y b r)
               (new-racket-vx b r)
               (new-racket-vy b r)
               (racket-selected? r)
...
;;; TESTS
(begin-for-test
  (check-equal? (new-racket (list B-MISS-R) R-MISS-B)
                (make-racket 350 375 0 10 false 0 0)
                "new-racket test 1 did not calculate correctly"))


;;; new-racket-x : BallList Racket -> Integer
;;; GIVEN: a BallList and a racket
...
;;; TESTS
(begin-for-test
  (check-equal? (new-racket-x (list B-RTS) R-COLLIDE-LEFT) 24
                "new-racket-x test 1 did not calculate correctly")
  (check-equal? (new-racket-x (list B-RTS) R-COLLIDE-RIGHT) 401
                "new-racket-x test 2 did not calculate correctly")
  (check-equal? (new-racket-x (list B-RTS) R-RTS) 330
                "new-racket-x test 3 did not calculate correctly")
  (check-equal? (new-racket-x (list B-RTS) (make-racket 50 60 70 80 true 0 0))
                50
                "new-racket-x test 4 did not calculate correctly"))


;;; new-racket-y : BallList Racket -> Integer
;;; GIVEN: a BallList and a racket
;;; RETURNS: the y-coordinate of the racket after 1 tick
;;; EXAMPLES:
;;; (new-racket-y B-COLLIDE-R R-COLLIDE-B) => 365
;;; (new-racket-y B-RTS R-RTS) => 384
;;; DESIGN STRATEGY: Cases on racket selected, then transcribe formula
...
;;; TESTS
(begin-for-test
  (check-equal? (new-racket-y (list B-COLLIDE-R) R-COLLIDE-B) 365
                "new-racket-y test 1 did not calculate correctly")
  (check-equal? (new-racket-y (list B-RTS) R-RTS) 384
                "new-racket-y test 2 did not calculate correctly")
  (check-equal? (new-racket-y (list B-RTS) (make-racket 50 60 70 80 true 0 0))
                60
                "new-racket-y test 3 did not calculate correctly"))

;;; new-racket-vx : BallList Racket -> Integer
;;; GIVEN: a BallList and a racket
...
;;; TESTS
(begin-for-test
  (check-equal? (new-racket-vx (list B-RTS) R-COLLIDE-LEFT) 0
                "new-racket-vx test 1 did not calculate correctly")
  (check-equal? (new-racket-vx (list B-RTS) R-COLLIDE-RIGHT) 0
                "new-racket-vx test 2 did not calculate correctly")
  (check-equal? (new-racket-vx (list B-RTS) R-RTS) 0
                "new-racket-vx test 3 did not calculate correctly"))


;;; new-racket-vy : BallList Racket -> Integer
;;; GIVEN: a BallList and a racket
;;; RETURNS: the y-coordinate of the racket after 1 tick
;;; EXAMPLES:
...
;;; DESIGN STRATEGY: If racket/ball collisions using HOF ormap, then
;;;                  transcribe formula
...
(define (new-racket-vy b r)
  (if (ormap
       ...
       (lambda (x) (ball-collide-racket? x r)) b)
      (if (< (racket-vy r) 0) 0 (racket-vy r))
      (racket-vy r)))

;;; (define (new-racket-vy b r)
;;;   (cond [(empty? b) (racket-vy r)]
;;;         [(ball-collide-racket? (first b) r)
;;;          (if (< (racket-vy r) 0) 0 (racket-vy r))]
;;;         [else (new-racket-vy (rest b) r)]))

;;; TESTS
(begin-for-test
  (check-equal? (new-racket-vy (list B-COLLIDE-R) R-COLLIDE-B) 0
                "new-racket-vy test 1 did not calculate correctly")
  (check-equal? (new-racket-vy (list B-COLLIDE-R)
                               (make-racket 350 365 5 -1 false 0 0))
                0
                "new-racket-vy test 2 did not calculate correctly")
  (check-equal? (new-racket-vy (list B-RTS) R-RTS) 0
...
(define (ball-collide-racket? b r)
  (and (> (racket-y r) (ball-y b))
       (or (= (ball-vy b) 0) (> (ball-vy b) 0))
       (ball-collide-racket-y? b r)
...
;;; ball-collide-racket-y? : Ball Racket -> Boolean
;;; GIVEN: a ball and a racket
;;; RETURNS: true if the ball collides with the racket
;;; EXAMPLE:
;;; (ball-collide-racket? B-COLLIDE-R R-COLLIDE-B) => true
...
(define (ball-collide-racket-y? b r)
  (if (racket-selected? r)
      (or (= (+ (ball-y b) (ball-vy b))
             (racket-y r))
          (> (+ (ball-y b) (ball-vy b))
             (racket-y r)))
      (or (= (+ (ball-y b) (ball-vy b))
...
;;; TESTS
(begin-for-test
  (check-equal? (ball-collide-racket-y? B-COLLIDE-R R-COLLIDE-B) true
                "ball-collide-racket-y? test 1 did not calculate correctly")
  (check-equal? (ball-collide-racket-y? B-COLLIDE-R
                                        (make-racket 350 365 5 0 true 0 0))
                true
                "ball-collide-racket-y? test 2 did not calculate correctly"))
...
;;; DESIGN STRATEGY:  Cases on ready-to-serve?
...
(define (world-after-rally-key-event w kev)
  (cond [(key=? kev " ") (world-reset w)]
        [(key=? kev "left") (world-arrow-press w kev)]
        [(key=? kev "right") (world-arrow-press w kev)]
        [(key=? kev "up") (world-arrow-press w kev)]
        [(key=? kev "down") (world-arrow-press w kev)]
        [(key=? kev "b") (world-new-ball w)]))
...
(begin-for-test
  ...
  (check-equal? (world-after-rally-key-event W-TEST "left")
                (world-arrow-press W-TEST "left")
                "world-after-rally-key-event test 2 not correct")
  (check-equal? (world-after-rally-key-event W-TEST "right")
                (world-arrow-press W-TEST "right")
                "world-after-rally-key-event test 3 not correct")
  (check-equal? (world-after-rally-key-event W-TEST "up")
                (world-arrow-press W-TEST "up")
                "world-after-rally-key-event test 4 not correct")
  (check-equal? (world-after-rally-key-event W-TEST "down")
                (world-arrow-press W-TEST "down")
                "world-after-rally-key-event test 5 not correct")
  (check-equal? (world-after-rally-key-event W-TEST "b")
                (world-new-ball W-TEST)
                "world-after-rally-key-event test 6 not correct"))


;;; world-arrow-press : World KeyEvent -> World
;;; GIVEN: a world and a key event
;;; RETURNS: the world with the key event applied
;;; EXAMPLE:
;;; (world-arrow-press W-TEST "left") =>
;;;                (make-world (world-balls W-TEST)
;;;                            (racket-arrow-press R-TEST "left")
;;;                            (world-speed W-TEST)
;;;                            (world-ready-to-serve? W-TEST)
;;;                            (world-paused? W-TEST)
;;;                            (world-plength W-TEST))
;;; DESIGN STRATEGY: Constructor template for World

(define (world-arrow-press w kev)
  (make-world (world-balls w)
              (racket-arrow-press (world-racket w) kev)
              (world-speed w)
              (world-ready-to-serve? w)
              (world-paused? w)
              (world-plength w)))
...
;;;TESTING
(begin-for-test
  (check-equal? (world-arrow-press W-TEST "left")
                (make-world (world-balls W-TEST)
                            (racket-arrow-press R-TEST "left")
                            (world-speed W-TEST)
                            (world-ready-to-serve? W-TEST)
                            (world-paused? W-TEST)
                            (world-plength W-TEST))
                "world-arrow-press test 1 not correct"))


;;; racket-arrow-press : Racket KeyEvent -> Racket
;;; GIVEN: a racket and a key event
;;; RETURNS: the racket with the key event applied
;;; EXAMPLE:
;;; (racket-arrow-press R-TEST "left") =>
;;;                (make-racket 50 60 69 80 false 0 0)
;;; DESIGN STRATEGY: Cases on key event

(define (racket-arrow-press r kev)
  (cond [(string=? kev "left") (racket-post-arrow r - 1 0)]
        [(string=? kev "right") (racket-post-arrow r + 1 0)]
        [(string=? kev "up") (racket-post-arrow r - 0 1)]
        [(string=? kev "down") (racket-post-arrow r + 0 1)]))

;;;TESTING
(begin-for-test
  (check-equal? (racket-arrow-press R-TEST "left")
                (make-racket 50 60 69 80 false 0 0)
                "racket-arrow-press test 1 not correct"))

;;; racket-post-arrow : Racket (Nat Nat -> Number) NonNegInt NonNegInt -> Racket
;;; GIVEN: a racket, a function (either + or -), and two non negative ints
;;; RETURNS: the racket with velocity adjusted by given inputs
;;; EXAMPLE: 
;;; (racket-post-arrow R-MISS-B - 1 0) => (make-racket 350 365 -1 10 false 0 0)
;;; (racket-post-arrow R-MISS-B + 1 0) => (make-racket 350 365 1 10 false 0 0)
;;; (racket-post-arrow R-MISS-B - 0 1) => (make-racket 350 365 0 9 false 0 0)
;;; (racket-post-arrow R-MISS-B + 0 1) => (make-racket 350 365 0 11 false 0 0)
;;; DESIGN STRATEGY: Constructor template for Racket

(define (racket-post-arrow r op vxamt vyamt)
  (make-racket (racket-x r)
               (racket-y r)
               (op (racket-vx r) vxamt)
               (op (racket-vy r) vyamt)
               false
               (racket-mx r)
               (racket-my r)))
...
;;;TESTING
(begin-for-test
  (check-equal? (racket-post-arrow R-MISS-B - 1 0)
                (make-racket 350 365 -1 10 false 0 0)
                "racket-post-arrow test 1 not correct")
  (check-equal? (racket-post-arrow R-MISS-B + 1 0)
                (make-racket 350 365 1 10 false 0 0)
                "racket-post-arrow test 1 not correct")
  (check-equal? (racket-post-arrow R-MISS-B - 0 1)
...
;;; world-new-ball : World -> World
;;; GIVEN: a world in a rally state
;;; RETURNS: that world with a new ball at (330,384) with velocity (3,-9)
;;; EXAMPLE: 
;;; (world-new-ball W-NO-COLL) => (make-world (make-ball 330 360 0 10)
...
(define (world-new-ball w)
  (make-world (cons B-RALLY (world-balls w))
              (world-racket w)
              (world-speed w)
              (world-ready-to-serve? w)
              (world-paused? w)
...
;;;TESTING
(begin-for-test
  (check-equal? (world-new-ball W-NO-COLL)
                (make-world (list (make-ball 330 384 3 -9)
                                  (make-ball 350 360 0 10))
                            (make-racket 350 365 0 10 false 0 0)
                            1
                            false
                            false
                            0)
                "world-new-ball test 1 not correct"))
...
(begin-for-test
  (check-equal? (world-after-mouse-event W-NO-COLL 350 350 "button-down")
                (make-world (list (make-ball 350 360 0 10))
