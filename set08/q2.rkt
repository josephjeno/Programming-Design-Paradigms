...
(define (power-ranking olist)
  (competitor-names (power-ranking-sort (outcome-cycler olist) olist)))
...
(begin-for-test
  (check-equal? (power-ranking (list (defeated "A" "D")
                                     ...
                                     (defeated "F" "E")))
                (list "C" "A" "F" "E" "B" "D")
                "power-ranking test 1 was not correct"))


;;; power-ranking-sort : CompetitorHistoryList -> CompetitorHistoryList
;;; GIVEN: an unsorted competitor history list
;;; RETURNS: the competitor history list sorted by power-ranking
;;; DESIGN STRATEGY: Use recursion and halt on list length

(define (power-ranking-sort chlist olist)
  (cond [(empty? chlist) empty]
        [else (cons (highest-ranking chlist olist)
                    (power-ranking-sort
                     (remove (highest-ranking chlist olist) chlist) olist))]))

;;; TESTS:
(begin-for-test
  (check-equal? (power-ranking-sort (list
                                     (make-comp-history "B" (list) (list "A"))
                                     (make-comp-history "A" (list "B") (list)))
                                    (list (defeated "A" "B")))
                (list
                 (make-comp-history "A" (list "B") (list))
                 (make-comp-history "B" (list) (list "A")))
                "power-ranking-sort test 1 was not correct"))

;;; highest-ranking : CompetitorHistoryList -> CompetitorHistory
;;; GIVEN: an unsorted competitor history list
;;; RETURNS: the highest ranked competitor history on the list
;;; DESIGN STRATEGY: Use HOF Foldr

(define (highest-ranking chlist olist)
  (foldr
   ;; CompetitorHistory CompetitorHistory -> CompetitorHistory
   (lambda (x r) (if (power-ranking-outranked? x r olist) x r)) empty chlist))

;;; TESTS:
(begin-for-test
  (check-equal? (highest-ranking (list
                                     (make-comp-history "B" (list) (list "A"))
                                     (make-comp-history "A" (list "B") (list)))
                                 (list (defeated "A" "B")))
                (make-comp-history "A" (list "B") (list))
                "highest-ranking test 1 was not correct"))

;;; power-ranking-outranked? : CompetitorHistory CompetitorHistory -> Boolean
;;; GIVEN: two arbitrary competitor historys
;;; RETURNS: true if the first competitor history outranks the second competitor
;;;          history according to the power-ranking-system
;;; DESIGN STRATEGY: Cases on power-ranking

(define (power-ranking-outranked? ch1 ch2 olist)
  (cond [(empty? ch2) true]
        [(< (length (comp-history-outranked ch1))
            (length (comp-history-outranked ch2))) true]
        [(> (length (comp-history-outranked ch1))
            (length (comp-history-outranked ch2))) false]
        [(> (length (comp-history-outranks ch1))
            (length (comp-history-outranks ch2))) true]
        [(< (length (comp-history-outranks ch1))
            (length (comp-history-outranks ch2))) false]
        [(> (non-losing-percentage ch1 olist)
            (non-losing-percentage ch2 olist)) true]
        [(< (non-losing-percentage ch1 olist)
            (non-losing-percentage ch2 olist)) false]
        [(string<? (comp-history-name ch1) (comp-history-name ch2)) true]
        [else false]))

;;; TESTS:
(begin-for-test
  (check-equal? (power-ranking-outranked? (make-comp-history "C"
                                                             (list "D")
                                                             (list "E"))
                                          (make-comp-history "A"
                                                             (list "D")
                                          ...
                                          (list (defeated "A" "C")))
                true
                "power-ranking-outranked? test 1 was not correct")
  (check-equal? (power-ranking-outranked? (make-comp-history "C"
                                                             (list "D")
                                                             (list "E"))
                                          (make-comp-history "A"
                                          ...
                                          (list (defeated "A" "C")))
                false
                "power-ranking-outranked? test 2 was not correct")
  (check-equal? (power-ranking-outranked? (make-comp-history "C"
                                                             (list "D")
                                                             (list "E"))
                                          (make-comp-history "A"
                                          ...
                                          (list (defeated "A" "C")))
                false
                "power-ranking-outranked? test 3 was not correct"))

;;; non-losing-percentage : CompetitorHistory -> Real
;;; GIVEN: an arbitrary competitor history
;;; RETURNS: the non-losing-percentage for the given competitor history
;;; DESIGN STRATEGY: Transcribe formula

(define (non-losing-percentage ch olist)
  (/ (number-outcomes-won-tied (comp-history-name ch) olist)
     (number-outcomes-mentioned (comp-history-name ch) olist)))

;;; TESTS:
(begin-for-test
  (check-equal? (non-losing-percentage
                 (make-comp-history "A" (list "B") (list))
                 (list (defeated "A" "B")))
                1
                "non-losing-percentage test 1 was not correct")
  (check-equal? (non-losing-percentage
                 (make-comp-history "A" (list) (list "B"))
                 (list (defeated "A" "B")))
                1
                "non-losing-percentage test 2 was not correct"))

;;; competitor-names : CompetitorHistoryList -> CompetitorList
;;; GIVEN: a competitor history list sorted by power-ranking
;;; RETURNS: a competitor list of all competitor history names
;;; DESIGN STRATEGY: Use HOF map

(define (competitor-names chlist)
  (map
   ;; CompetitorHistory -> Competitor
   (lambda (x) (comp-history-name x)) chlist))

;;; TESTS:
(begin-for-test
  (check-equal? (competitor-names (list
                                     (make-comp-history "B" (list) (list "A"))
                                     (make-comp-history "A" (list "B") (list))))
                (list "B" "A")
                "competitor-names test 1 was not correct"))

;;; number-outcomes-mentioned : Competitor OutcomeList -> NonNegInt

(define (number-outcomes-mentioned comp olist)
  (foldr (lambda (x r) (if (defeat-type? x)
                           (if (or (string=? (defeat-type-winner x) comp)
                                   (string=? (defeat-type-loser x) comp))
                               (+ r 1)
                               r)
                           (if (or (string=? (tie-type-comp1 x) comp)
                                   (string=? (tie-type-comp2 x) comp))
                               (+ r 1)
                               r))) 0 olist))

;;; number-outcomes-won-tied : Competitor OutcomeList -> NonNegInt

(define (number-outcomes-won-tied comp olist)
  (foldr (lambda (x r) (if (defeat-type? x)
                           (if (string=? (defeat-type-winner x) comp)
                               (+ r 1)
                               r)
                           (if (or (string=? (tie-type-comp1 x) comp)
                                   (string=? (tie-type-comp2 x) comp))
